...
def train(model, train_stream, val_stream, task_id, noise_var, args):
    ...
    # load the forward model ----> this is needed when we connect it with the PNN model
    ...
    for epoch in range(args.epochs):
        ...
        for features, targets in train_loader:
            ...
            if task_id==0:            
                X_batch = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init[-X_batch.shape[0]:, :, :], error_fb_seg0[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                ...
                seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init[-X_batch.shape[0]:, :, :], error_fb_seg0[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                with torch.no_grad():
                    model.eval()
                    out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                    model.train()
                # current model
                X_batch = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init[-X_batch.shape[0]:, :, :], error_fb_seg1[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                ...
                seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init[-X_batch.shape[0]:, :, :], error_fb_seg0[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                #seg1
                seg1_fb_input = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init[-X_batch.shape[0]:, :, :], error_fb_seg1[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                with torch.no_grad():
                    model.eval()
                    out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                    ...
                    model.train()
                # current model
                X_batch = torch.concat((features[:, 2, :, 0:feature_dim].to(device), act2_init[-X_batch.shape[0]:, :, :], error_fb_seg2[-X_batch.shape[0]:, :, :]), dim=2).to(device)
            ...
            # # take the feedback from the forward model
            # # extract the s0, s1 information from the features to pass it to the forward model

            # forward model precictions
            with torch.no_grad():

                if task_id==0:
                    # s0_pose_train_seg0 = features[:, 0, :, feature_dim : feature_dim+6].to(device)
                    # s1_pose_train_seg0 = features[:, 0, :, feature_dim+6 : ].to(device)
                    ...
                    # s0_pose_train_seg1 = features[:, 1, :, feature_dim : feature_dim+6].to(device)
                    # s1_pose_train_seg1 = features[:, 1, :, feature_dim+6 : ].to(device)
                    ...
                    # s0_pose_train_seg2 = features[:, 2, :, feature_dim : feature_dim+6].to(device)
                    # s1_pose_train_seg2 = features[:, 2, :, feature_dim+6 : ].to(device)
            ...
            # store the error from the current batch of FM to send it as input to the next batch
            error_fb_seg0 = torch.abs(features[:, 0, :, 0:6].cuda().detach() - fm_feedback_train_seg0.detach())
            error_fb_seg1 = torch.abs(features[:, 1, :, 0:6].cuda().detach() - fm_feedback_train_seg1.detach()) if fm_feedback_train_seg1 is not None else 0
            error_fb_seg2 = torch.abs(features[:, 2, :, 0:6].cuda().detach() - fm_feedback_train_seg2.detach()) if fm_feedback_train_seg2 is not None else 0   
        ...
        with torch.no_grad():
            ...
            for features, targets in val_loader:
                ...
                # Add the feedback from the forward model
                if flag_val:
                    error_fb_seg0_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device) # orientation is true
                    error_fb_seg1_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)
                    error_fb_seg2_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)
                    ...
                    s0_pose_mod1_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)
                    s1_pose_mod1_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)

                    s0_pose_mod2_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)
                    s1_pose_mod2_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)

                    s0_pose_mod3_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)
                    s1_pose_mod3_val = torch.zeros((X_batch.shape[0], args.time_step, 6)).to(device)

                    flag_val = False

                if task_id==0:
                    X_batch = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init_val[-X_batch.shape[0]:, :, :], error_fb_seg0_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                    ...
                    # needs actuation from the first module to send it to the forward model of module 2
                    # previous seg prediction
                    seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init_val[-X_batch.shape[0]:, :, :], error_fb_seg0_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                    # seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                    out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                    # # current model
                    X_batch = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init_val[-X_batch.shape[0]:, :, :], error_fb_seg1_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                    ...
                    out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                    #seg1
                    seg1_fb_input = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init_val[-X_batch.shape[0]:, :, :], error_fb_seg1_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                    out_seg1, _, _ = model(seg1_fb_input, 0, 0, 1) # last is task id
                    # current model
                    X_batch = torch.concat((features[:, 2, :, 0:feature_dim].to(device), act2_init_val[-X_batch.shape[0]:, :, :], error_fb_seg2_val[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                ...
                with torch.no_grad():
                    ...
                    if task_id==0:
                        # s0_pose_train_seg0 = features[:, 0, :, feature_dim : feature_dim+6].to(device)
                        # s1_pose_train_seg0 = features[:, 0, :, feature_dim+6 : ].to(device)
                        fm_feedback_train_seg0 = forward_model_seg0(torch.concat((s0_pose_mod1_val[-X_batch.shape[0]:, :, :], s1_pose_mod1_val[-X_batch.shape[0]:, :, :], outputs), dim=2))
                        ...
                        fm_feedback_train_seg0 = forward_model_seg0(torch.concat((s0_pose_mod1_val[-X_batch.shape[0]:, :, :], s1_pose_mod1_val[-X_batch.shape[0]:, :, :], out_seg0), dim=2))
                        fm_feedback_train_seg1 = forward_model_seg1(torch.concat((s0_pose_mod2_val[-X_batch.shape[0]:, :, :], s1_pose_mod2_val[-X_batch.shape[0]:, :, :], out_seg0, outputs), dim=2))
                        ...
                        fm_feedback_train_seg0 = forward_model_seg0(torch.concat((s0_pose_mod1_val[-X_batch.shape[0]:, :, :], s1_pose_mod1_val[-X_batch.shape[0]:, :, :], out_seg0), dim=2))
                        fm_feedback_train_seg1 = forward_model_seg1(torch.concat((s0_pose_mod2_val[-X_batch.shape[0]:, :, :], s1_pose_mod2_val[-X_batch.shape[0]:, :, :], out_seg0, out_seg1), dim=2))
                        fm_feedback_train_seg2 = forward_model_seg2(torch.concat((s0_pose_mod3_val[-X_batch.shape[0]:, :, :], s1_pose_mod3_val[-X_batch.shape[0]:, :, :], out_seg0, out_seg1, outputs), dim=2))


                loss = criterion(outputs, Y_batch)
                val_loss += loss.item()

                # store the error from the current batch of FM to send it to the next batch ---> validation
                error_fb_seg0_val = torch.abs(features[:, 0, :, 0:6].cuda().detach() - fm_feedback_train_seg0.detach())
                error_fb_seg1_val = torch.abs(features[:, 1, :, 0:6].cuda().detach() - fm_feedback_train_seg1.detach()) if fm_feedback_train_seg1 is not None else 0
                error_fb_seg2_val = torch.abs(features[:, 2, :, 0:6].cuda().detach() - fm_feedback_train_seg2.detach()) if fm_feedback_train_seg2 is not None else 0  

                if task_id == 0:
                    act0_init_val = outputs.detach()
                    # position and orientation
                    s1_pose_mod1_val = s0_pose_mod1_val.detach()
                    s0_pose_mod1_val = fm_feedback_train_seg0.detach()
                elif task_id == 1:
                    act0_init_val = out_seg0.detach()
                    ...
                    s0_pose_mod1_val = fm_feedback_train_seg0.detach()
                    # mod 2
                    s1_pose_mod2_val = s0_pose_mod2_val.detach()
                    s0_pose_mod2_val = fm_feedback_train_seg1.detach()

                elif task_id == 2:
                    act0_init_val = out_seg0.detach()
                    act1_init_val = out_seg1.detach()
                    ...
                    s0_pose_mod2_val = fm_feedback_train_seg1.detach()
                    # mod 3
                    s1_pose_mod3_val = s0_pose_mod3_val.detach()
                    s0_pose_mod3_val = fm_feedback_train_seg2.detach()
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # save the best model
            torch.save(model.state_dict(),  'saved_models_plus_custom_shape/model_only_once' + '.pt')
...
def test(model, test_size, test_stream, task_id, noise_var, args):
    ...
    model.load_state_dict(torch.load('saved_models_plus_custom_shape/model' + '.pt', map_location=device))
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        ...
        for features, targets in test_loader:    
            ...
            if task_id==0:
                X_batch = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init, error_fb_seg0[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                # X_batch = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                Y_batch = targets[:, 0, :, 0:target_dim].to(device)
                ...
                out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                # current model
                X_batch = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init, error_fb_seg1[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                # X_batch = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                Y_batch = targets[:, 1, :, 0:target_dim].to(device)
                ...
                # needs actuation from the first module, second module to send it to the forward model of module 3
                #seg 0
                seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init, error_fb_seg0[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                # seg0_fb_input = torch.concat((features[:, 0, :, 0:feature_dim].to(device), act0_init), dim=2).to(device)
                out_seg0, _, _ = model(seg0_fb_input, 0, 0, 0) # last is task id
                #seg1
                seg1_fb_input = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init, error_fb_seg1[-X_batch.shape[0]:, :, :]), dim=2).to(device)
                # seg1_fb_input = torch.concat((features[:, 1, :, 0:feature_dim].to(device), act1_init), dim=2).to(device)
                out_seg1, _, _ = model(seg1_fb_input, 0, 0, 1) # last is task id
                # current model
                X_batch = torch.concat((features[:, 2, :, 0:feature_dim].to(device), act2_init, error_fb_seg2[-X_batch.shape[0]:, :, :]), dim=2).to(device)
            ...
            # # store the error from the current batch of FM to send it to the next batch
            error_fb_seg0 = torch.abs(features[:, 0, :, 0:6].cuda().detach() - fm_feedback_train_seg0.detach())
            error_fb_seg1 = torch.abs(features[:, 1, :, 0:6].cuda().detach() - fm_feedback_train_seg1.detach()) if fm_feedback_train_seg1 is not None else 0
            error_fb_seg2 = torch.abs(features[:, 2, :, 0:6].cuda().detach() - fm_feedback_train_seg2.detach()) if fm_feedback_train_seg2 is not None else 0
