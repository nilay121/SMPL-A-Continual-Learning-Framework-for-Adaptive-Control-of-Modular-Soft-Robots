...
from train_test_closed_loop import train, test
...
def incremental_call(model, X, Y, batch_size, task_id, noise_var, args):
    ...
    if args.mode == "train" or args.mode == "test":        
        ...
        # train
        train_X, train_Y = X[:12000], Y[:12000] 
        # val
        val_X, val_Y = X[12000:12000 + 500], Y[12000:12000 + 500]
        # test
        test_size = 1500
        test_X, test_Y = X[12500:], Y[12500:]
    ...
    if args.mode == "train":
        ...
        test_stream = CustomDatasetForDataLoader(X[12500:], Y[12500:])
        test_size=1500
        
        print("------------------------->Starting testing for task-id<-------------------------", task_id)
        pred, gt, fm_pred, fm_gt, _, _ = test(model=model, test_size=test_size, test_stream=test_stream,
                        task_id=task_id, noise_var=noise_var, args=args)
        act_plot(args, pred, gt, task_id, samples=200) 
        TSTime(args, fm_pred, fm_gt, task_id, samples=200)
        ...
        TSTime(args, fm_pred, fm_gt, task_id, samples=1000)
...
def main(args):
    ...
    # model init
    model = ProgNet(colGen = PNN_model(input_dim=feature_dim+fb_dim+4, out_dim=target_act_dim, hidden_size=args.hidden_size, 
